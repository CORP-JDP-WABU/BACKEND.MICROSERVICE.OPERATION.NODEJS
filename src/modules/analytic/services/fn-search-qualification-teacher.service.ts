...
export class FnSearchQualificationTeachersService { 
    ...
    async execute(request: interfaces.IGlobalAnalityc) {
        ...
        const analitycSearch = await this.analitycSearchQualificationTeacherModel.findOne({
    ...
    private async update(idAnalityc: mongoose.Types.ObjectId) {
        const update = await this.analitycSearchQualificationTeacherModel.findByIdAndUpdate(idAnalityc, {
            $set: {
