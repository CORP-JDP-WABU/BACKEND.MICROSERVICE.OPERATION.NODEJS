...
export class FnCommentService {
    ...
    async execute(idCourse: string, idTeacher: string, userDecorator: any, comment: string) {
        ...
        const student = await this.studentModel.findById(idStudent, { _id: 1, firstName: 1, lastName: 1, university: 1 });
    ...
    private async createCommentInCourseTeacher(student: any, idTeacher: string, idCourse: string, comment: string) {
